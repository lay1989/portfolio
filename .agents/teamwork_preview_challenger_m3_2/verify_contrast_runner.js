...
// Custom Tailwind classes used in the project
const CUSTOM_COLORS = {
    text: {
...
filesToCheck.forEach(filePath => {
    ...
    icons.each((idx, elem) => {
        ...
        if (!lightBgHex) {
            ...
            // It's a blend. We need to find the base background below it.
        ...
        if (!darkBgHex) {
            if (lightBgClass && lightBgClass !== 'default-page-bg') {
                if (typeof CUSTOM_COLORS.bg[lightBgClass] === 'object') {
                    // It's a custom color or opacity blend
        ...
        // 1. Text color is #FF6B35 on #F5F0EA or #EDE8E1 backgrounds (any contrast or color check)
        const isOrange = lightTextHex === '#FF6B35';
        const isCreamBg = lightBgHex === '#F5F0EA' || lightBgHex === '#EDE8E1';
        
        if (isOrange && isCreamBg) {
            violations.push({
                type: 'ORANGE_ON_CREAM_VIOLATION',
                icon: iconInfo,
                message: `Icon renders as #FF6B35 on background ${lightBgHex} (${lightBgClass})`
            });
        }
        
        // 2. Light mode contrast ratio < 3.0:1
        if (lightContrast < 3.0) {
            violations.push({
                type: 'LIGHT_MODE_CONTRAST_VIOLATION',
                icon: iconInfo,
                message: `Light mode contrast ratio is ${lightContrast}:1 (< 3.0:1) with text ${lightTextHex} on bg ${lightBgHex}`
            });
        }
        
        // 3. Dark mode contrast ratio < 3.0:1
        if (darkContrast < 3.0) {
            violations.push({
                type: 'DARK_MODE_CONTRAST_VIOLATION',
                icon: iconInfo,
                message: `Dark mode contrast ratio is ${darkContrast}:1 (< 3.0:1) with text ${darkTextHex} on bg ${darkBgHex}`
            });
        }
    });
});
